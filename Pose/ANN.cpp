...
#define OLN 4 //Number of output nodes
#define HLN 6 //Number of hidden layer nodes
#define IN 960 //Length of input vector
int inpn = 277; //Number of training examples
...
double testinp1[139][IN+1], testout1[139][OLN], testinp2[208][IN+1], testout2[208][OLN];
int epochs = 1000;
...
{	
	...
	getAcc(testinp1, testout1, 139);
	cout << "Testing set 2 accuracy is: ";
	getAcc(testinp2, testout2, 208);
}
...
{	
	...
	//load data in matrix
	ifstream myfile("trainingInpP.txt");
	ifstream file("trainingOutP.txt");
	ss << myfile.rdbuf();
	...
	//load testing data 1
	myfile.open("testInpP1.txt");
	file.open("testOutP1.txt");
	ss << myfile.rdbuf();
	...
	//load testing data 2
	myfile.open("testInpP2.txt");
	file.open("testOutP2.txt");
	ss << myfile.rdbuf();
