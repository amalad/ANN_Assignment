...
#define OLN 2 //Number of output nodes
#define HLN 5 //Number of hidden layer nodes
#define IN 960 //Length of input vector
int inpn = 70; //Number of training examples
...
double testinp1[34][IN+1], testout1[34][OLN], testinp2[52][IN+1], testout2[52][OLN];
int epochs = 500;
...
{	
	...
	getAcc(testinp1, testout1, 34);
	cout << "Testing set 2 accuracy is: ";
	getAcc(testinp2, testout2, 52);
}
...
{	
	...
	//load data in matrix
	ifstream myfile("trainingInpS.txt");
	ifstream file("trainingOutS.txt");
	ss << myfile.rdbuf();
	...
	//load testing data 1
	myfile.open("testInpS1.txt");
	file.open("testOutS1.txt");
	ss << myfile.rdbuf();
	...
	//load testing data 2
	myfile.open("testInpS2.txt");
	file.open("testOutS2.txt");
	ss << myfile.rdbuf();
